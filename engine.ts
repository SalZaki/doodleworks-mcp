/**
 * engine.ts — image generation for the Doodleworks MCP MCP App.
 *
...
export const TINKU_CHARACTER =
  "Tinku is a small, boldly simple ink-drawn worker — NOT a standard little person with a separate head, " +
  "neck, and torso. His whole body is ONE continuous solid matte-black shape: a single rounded egg/teardrop " +
  "blob, slightly taller than wide, chunky and grounded, with NO neck and NO separate head — the head and the " +
  "body are the same mass. He is filled with confident solid matte black and drawn with the same clean, smooth, " +
  "even black line as the rest of the page (deliberate and tidy, not sketchy or wobbly). " +
  "His face sits high on that mass and is EXPRESSIVE and reactive (not deadpan): two large round white eyes that " +
  "widen, squint, or strain with the effort of the task, and a simple mouth showing concentration, surprise, or " +
  "resolve. On the crown he has a signature tuft of EXACTLY three short upward ink sprigs — small and always " +
  "present, never a full head of hair. " +
  "He has EXACTLY two arms and EXACTLY two legs and nothing more. The arms are short, simple, and slightly " +
  "tapering, set high on the blob, each ending in one small rounded mitten-hand built for gripping; relaxed " +
  "arm length is about half the body height — never long or thin. The legs are two short stubby legs, slightly " +
  "splayed for a planted stance. He is forever pulling levers, turning cranks, feeding hoppers, propping things " +
  "up, plugging leaks. " +
  "Limb discipline: never a third arm or leg, never duplicated, forked, or branching limbs, never an extra hand. " +
  "Because he is a solid black shape, keep his arms and legs simple and well-separated so they never merge into " +
  "each other, into a tangle, or into the contraption he is operating. " +
  "Genderless, culture-neutral, no logos or text on him. " +
  "Defining rule: Tinku is NEVER a mascot, sticker, or corner decoration — he is the absurd " +
  "worker physically OPERATING the contraption that embodies the article's idea. Test: if you " +
  "could erase Tinku and the picture still made complete sense on its own, he was merely " +
  "decorative and the composition has failed — redraw it so the idea only works because he is " +
  "doing something to it. At most one Tinku per scene, drawn small so the contraption and the " +
  "whitespace dominate the frame.";
...
export interface RenderOptions {
   ...
   * Defaults to the env DOODLEWORKS_QUALITY, else "medium". Ignored by the Gemini provider.
   */
  quality?: Quality;
}
